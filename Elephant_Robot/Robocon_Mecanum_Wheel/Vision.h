...
void buttonPressed() {
  if (Ps3.event.button_down.square & !square) {
    // Serial.println("Square Button pressed"); 
    square = true;}  
  if (Ps3.event.button_down.triangle & !triangle) {
    // Serial.println("Triangle Button pressed"); 
    triangle = true;}
  if (Ps3.event.button_down.cross & !cross) {
    // Serial.println("Cross Button pressed"); 
    cross = true;}
  if (Ps3.event.button_down.circle & !circle) {
...
void vision_setup() {
  ...
  PS3_setup();
  PWMsetup(0, 19);  //set pin 26 to servo pin, channel 0
  delay(100);
  setPos(0, 90);
}
...
//    Serial.println(received);
    ...
    else if (received == "%") {currentArr = 1; dataReceived = true; requesting = false; 
    //printArr();
    }
    else if (received == "^") {resetArr(); currentArr = 1; 
    // Serial.println("Arrays cleared"); 
    dataReceived = false;}
...
void navigation() {
  if (square & arr1Size > 0) {
    ...
    // call arr1 for distance, arr2 for angle
    // arr1 needs abs, - to indicate left 
